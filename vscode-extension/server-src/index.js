        ...
        inputSchema: {
          ...
          properties: {
            ...
            compact: {
              type: "boolean",
              description:
                "Se true (padrão), retorna apenas metadata. Se false, inclui descrição completa.",
            },
          },
        },
      },
        ...
        inputSchema: {
          ...
          properties: {
            ...
            path: {
              type: "string",
              description:
                "Caminho absoluto do arquivo (opcional, usa padrão do provider)",
            },
            migrate: {
              type: "boolean",
              description:
                "Se true, migra dados do storage atual (default: false)",
            },
          },
          required: ["provider"],
        },
  ...
  try {
    ...
    switch (name) {
      case "add_prompt": {
        ...
        // JSON format for VS Code extension
        if (useJson) {
          return {
            content: [
      ...
      case "list_prompts": {
        ...
        const useJson = params.format === "json"; // JSON format for programmatic access
        // Aplicar filtros
        if (params.category) {
      ...
      case "get_prompt": {
        ...
        if (!prompt) {
          ...
          throw new McpError(ErrorCode.InvalidRequest, "Prompt não encontrado");
        }
        // JSON format for VS Code extension
        if (useJson) {
          return {
          ...
          content: [
            {
              ...
              text:
                ...
                (config.metadata?.deviceName
                  ? `**Dispositivo:** ${config.metadata.deviceName}\n`
                  : ""),
            },
          ],
        };
      ...
      case "configure_storage": {
        ...
        if (!targetProvider) {
              ...
              content: [
                {
                  ...
                  text: JSON.stringify({
                    ...
                    message: `Provider "${params.provider}" não reconhecido`,
          ...
          throw new McpError(
            ErrorCode.InvalidRequest,
            `Provider "${params.provider}" não reconhecido. Disponíveis: ${availableProviders}`,
          );
        }
        if (!targetProvider.available) {
              ...
              content: [
                {
                  ...
                  text: JSON.stringify({
                    ...
                    message: `${friendlyName} não está instalado ou não foi detectado neste sistema`,
                    provider: params.provider,
                    suggestion:
                      "Instale o aplicativo desktop ou use o armazenamento local",
          ...
          const alternativesMsg =
            availableProviders.length > 1
              ? `\n\nAlternativas disponíveis: ${availableProviders.map((p) => p.name).join(", ")}`
              : "";
          throw new McpError(
            ErrorCode.InvalidRequest,
            `${friendlyName} não está instalado ou não foi detectado neste sistema. ` +
              `Instale o aplicativo desktop do ${friendlyName} ou use o armazenamento local.${alternativesMsg}`,
          );
        }
